...
@app.cell
def _(np):
    def generate_points(count, seed=42):
        """Generate random scatterplot points using numpy.
        ...
        radii = rng.uniform(100, 5000, count).astype(np.float32)
        colors_rgb = rng.integers(100, 256, (count, 3), dtype=np.uint8)
...
@app.cell
def _(generate_points, mo, point_count_dropdown, time):
    count = point_count_dropdown.value
    _t0 = time.perf_counter()
    points, points_arrays = generate_points(count)
    gen_time_ms = (time.perf_counter() - _t0) * 1000

    mo.md(f"Generated **{len(points):,}** points in **{gen_time_ms:.0f} ms**")
    return count, gen_time_ms, points, points_arrays
...
def _(
    ...
    _layer = dgl.ScatterplotLayer(
        get_fill_color=[100, 150, 200, 180] if const_color_switch.value else "color",
        get_radius="radius",
        radius_scale=10,
        radius_min_pixels=2,
        radius_max_pixels=10,
    ...
    _arrays = points_arrays
    attrs = {
        "getPosition": (_arrays["positions"], "float32", 2),
        "getFillColor": (None if const_color_switch.value else _arrays["colors"], "uint8", 4),
        "getRadius": (_arrays["radii"], "float32", 1),
    }
    # Remove None entries
    attrs = {k: v for k, v in attrs.items() if v[0] is not None}
